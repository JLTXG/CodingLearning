本文题解基于以下原题整理，题意经本人重新表述，解题思路与代码均为原创或转载，仅用于个人学习与交流，完整题目请务必查阅官方原题链接：[3721. 最长平衡子数组 II - 力扣（LeetCode）](https://leetcode.cn/problems/longest-balanced-subarray-ii/description/?envType=daily-question&envId=2026-02-11)

>标签：高级工程师  |  线段树  |  数组  |  哈希表  |  分治  |  前缀和

# 一、题目描述

## **问题概括**

给定一个整数数组 `nums`，对于一个连续非空的子数组，如果其中包含的**不同偶数值**的数量等于**不同奇数值**的数量，则称该子数组是“平衡的”。请找出并返回数组中**最长平衡子数组**的长度。

## **示例说明**

- **示例1**：`nums = [2,5,4,3]`，整个数组包含两个不同的偶数（2 和 4）和两个不同的奇数（5 和 3），因此最长平衡子数组长度为 4。
- **示例2**：`nums = [3,2,2,5,4]`，整个数组包含两个不同的偶数（2 和 4）和两个不同的奇数（3 和 5），长度为 5。
- **示例3**：`nums = [1,2,3,2]`，子数组 `[2,3,2]` 包含一个不同的偶数（2）和一个不同的奇数（3），长度为 3。

## **数据范围**

- 数组长度 `n` 满足 `1 ≤ n ≤ 10^5`。
- 数组中的每个元素满足 `1 ≤ nums[i] ≤ 10^5`。



# 二、解题思路\解题过程

## ✅ 带详细注释的完整代码

### 🧩 先理解题意

> **“平衡子数组”定义**：子数组中 **奇数个数 = 偶数个数**。
> 即：若把奇数看作 `+1`，偶数看作 `-1`，则子数组的**和为 0**。

> **但本题有额外限制**：子数组中**每个数字最多出现两次**！
> 换句话说：子数组中不能有某个数字出现 **3 次或以上**。

所以问题转化为：
**找最长的子数组 `[l, r]`，使得：**

1. 子数组的“奇偶和”为 0（即 `prefix[r+1] == prefix[l]`）；
2. 子数组中任意数字出现次数 ≤ 2。

------

### 📝 注释版代码

```java
// 懒标记线段树：支持区间加、查询某值首次出现位置
class LazySegmentTree {
    private static final class Node {
        int min;   // 当前区间最小值
        int max;   // 当前区间最大值
        int todo;  // 懒标记（待下传的增量）
    }

    // 将懒标记 todo 应用到 node 节点
    private void apply(int node, int todo) {
        Node cur = tree[node];
        cur.min += todo;
        cur.max += todo;
        cur.todo += todo; // 累加懒标记（因为可能多次更新）
    }

    private final int n;
    private final Node[] tree;

    // 构造函数：维护长度为 n 的数组（下标 0 ~ n-1）
    public LazySegmentTree(int n) {
        this.n = n;
        // 分配足够空间：2 << ceil(log2(n))，确保是满二叉树
        tree = new Node[2 << (32 - Integer.numberOfLeadingZeros(n - 1))];
        Arrays.setAll(tree, _ -> new Node());
    }

    // 区间更新：对 [ql, qr] 每个元素加 f
    public void update(int ql, int qr, int f) {
        update(1, 0, n - 1, ql, qr, f);
    }

    // 查询：在 [ql, qr] 中第一个值等于 target 的下标，不存在返回 -1
    public int findFirst(int ql, int qr, int target) {
        return findFirst(1, 0, n - 1, ql, qr, target);
    }

    // 下传懒标记到左右子节点
    private void spread(int node) {
        int todo = tree[node].todo;
        if (todo == 0) return;
        apply(node * 2, todo);       // 左儿子
        apply(node * 2 + 1, todo);   // 右儿子
        tree[node].todo = 0;         // 清空当前节点懒标记
    }

    // 合并左右子节点信息到当前节点
    private void maintain(int node) {
        tree[node].min = Math.min(tree[node * 2].min, tree[node * 2 + 1].min);
        tree[node].max = Math.max(tree[node * 2].max, tree[node * 2 + 1].max);
    }

    // 递归区间更新
    private void update(int node, int l, int r, int ql, int qr, int f) {
        if (ql <= l && r <= qr) { // 完全覆盖
            apply(node, f);
            return;
        }
        spread(node); // 下传懒标记
        int m = (l + r) / 2;
        if (ql <= m) {
            update(node * 2, l, m, ql, qr, f);
        }
        if (qr > m) {
            update(node * 2 + 1, m + 1, r, ql, qr, f);
        }
        maintain(node); // 更新当前节点信息
    }

    // 递归查找第一个等于 target 的位置
    private int findFirst(int node, int l, int r, int ql, int qr, int target) {
        // 剪枝：区间不交 或 target 不在 [min, max] 范围内
        if (l > qr || r < ql || target < tree[node].min || target > tree[node].max) {
            return -1;
        }
        if (l == r) { // 叶子节点，必然是答案
            return l;
        }
        spread(node);
        int m = (l + r) / 2;
        // 优先查左子树（保证“第一个”）
        int idx = findFirst(node * 2, l, m, ql, qr, target);
        if (idx < 0) {
            idx = findFirst(node * 2 + 1, m + 1, r, ql, qr, target);
        }
        return idx;
    }
}

class Solution {
    public int longestBalanced(int[] nums) {
        int n = nums.length;
        // 线段树维护前缀和数组 sum[0..n]，共 n+1 个元素
        LazySegmentTree t = new LazySegmentTree(n + 1);

        // last[x] = x 上一次出现的位置（1-indexed）
        Map<Integer, Integer> last = new HashMap<>();
        int ans = 0;
        int curSum = 0; // 当前前缀和（从 sum[0]=0 开始）

        // i 从 1 到 n，对应 nums[i-1]
        for (int i = 1; i <= n; i++) {
            int x = nums[i - 1];
            // 将奇数映射为 +1，偶数映射为 -1
            int v = (x % 2 == 1) ? 1 : -1;

            Integer j = last.get(x);
            if (j == null) {
                // 首次遇到 x：从位置 i 开始，所有后续前缀和都要 +v
                curSum += v;
                t.update(i, n, v); // sum[i..n] += v
            } else {
                // 再次遇到 x：撤销上一次对 [j, i-1] 的 +v（因为现在 x 出现两次，中间部分不能再贡献）
                t.update(j, i - 1, -v);
                // 注意：第三次及以后出现时，会继续撤销之前的贡献，确保只有最近两次有效
            }
            last.put(x, i); // 更新 x 的最新位置

            // 关键优化：只在 [0, i-1-ans] 范围内查找
            // 因为我们要找更长的子数组，所以 l 必须 ≤ i-1-ans，否则长度 ≤ ans
            int l = t.findFirst(0, i - 1 - ans, curSum);
            if (l >= 0) {
                ans = i - l; // 子数组 [l, i-1] 长度为 i - l
            }
        }
        return ans;
    }
}
```

------

## 🧠 核心解题思想深度解析

### 🔑 核心洞察 1：**“有效前缀和”的动态维护**

- 正常情况下，若 `sum[r+1] == sum[l]`，则 `[l, r]` 是平衡子数组。
- 但本题要求：子数组中每个数字最多出现 **2 次**。
- 所以，**不能简单用静态前缀和**，因为当某个数字出现第 3 次时，之前的部分前缀和就“失效”了。

> 💡 解决方案：**动态维护一个“有效前缀和”数组 `sum`**，其中：
>
> - `sum[i]` 表示从开头到位置 `i-1` 的“有效奇偶和”；
> - 当某个数字 `x` 第三次出现时，要**撤销它第二次出现时对中间区间的贡献**。

### 🔑 核心洞察 2：**懒标记线段树的作用**

线段树在这里维护的是 **动态前缀和数组 `sum[0..n]`**，支持：

- **区间加**：当新数字加入或旧贡献被撤销时，批量更新 `sum`；
- **按值查询**：快速找到 `sum[l] == curSum` 的最小 `l`（即最远的起点）。

> 为什么需要线段树？
> 因为前缀和在不断变化（撤销操作），无法用普通哈希表记录。

### 🔑 核心洞察 3：**贪心优化搜索范围**

```java
int l = t.findFirst(0, i - 1 - ans, curSum);
```

- 我们只关心能**让答案变大**的 `l`；
- 当前最长是 `ans`，所以新子数组长度必须 > `ans` ⇒ `i - l > ans` ⇒ `l < i - ans`；
- 因此只需在 `[0, i-1-ans]` 中查找，大幅减少查询范围。

------

## 📚 笔记总结

### 🧩 问题定义：

找最长子数组，满足：

1. 奇数个数 = 偶数个数（即“奇偶和”为 0）；
2. 任意数字出现次数 ≤ 2。

### 💡 核心思想：

> **动态前缀和 + 懒标记线段树 + 贪心剪枝**

#### 步骤分解：

1. **映射**：奇数 → `+1`，偶数 → `-1`；
2. 动态维护前缀和 `sum`：
    - 首次见 `x`：从当前位置开始，所有后续 `sum` 加 `v`；
    - 再次见 `x`：撤销上一次对 `[last[x], i-1]` 的加法（因为现在 `x` 出现两次，中间部分仍有效）；
    - 第三次见 `x`：会再次撤销，确保只有最近两次贡献有效；
3. 线段树支持：
    - 区间加（`update`）；
    - 查询某值首次出现位置（`findFirst`）；
4. **贪心优化**：只在 `[0, i-1-ans]` 中查找，保证新答案更优。

### ⏱️ 复杂度：

- 时间：O(n log n) —— 每次更新/查询 O(log n)
- 空间：O(n) —— 线段树空间

### 🔑 关键技巧：

- **懒标记线段树维护动态数组**；
- **通过撤销操作实现“最多两次”约束**；
- **利用“答案单调不减”进行搜索剪枝**。

---

# 三、代码演示

## Java代码

```Java
// 懒标记线段树：支持区间加、查询某值首次出现位置
class LazySegmentTree {
    private static final class Node {
        int min;   // 当前区间最小值
        int max;   // 当前区间最大值
        int todo;  // 懒标记（待下传的增量）
    }

    // 将懒标记 todo 应用到 node 节点
    private void apply(int node, int todo) {
        Node cur = tree[node];
        cur.min += todo;
        cur.max += todo;
        cur.todo += todo; // 累加懒标记（因为可能多次更新）
    }

    private final int n;
    private final Node[] tree;

    // 构造函数：维护长度为 n 的数组（下标 0 ~ n-1）
    public LazySegmentTree(int n) {
        this.n = n;
        // 分配足够空间：2 << ceil(log2(n))，确保是满二叉树
        tree = new Node[2 << (32 - Integer.numberOfLeadingZeros(n - 1))];
        Arrays.setAll(tree, _ -> new Node());
    }

    // 区间更新：对 [ql, qr] 每个元素加 f
    public void update(int ql, int qr, int f) {
        update(1, 0, n - 1, ql, qr, f);
    }

    // 查询：在 [ql, qr] 中第一个值等于 target 的下标，不存在返回 -1
    public int findFirst(int ql, int qr, int target) {
        return findFirst(1, 0, n - 1, ql, qr, target);
    }

    // 下传懒标记到左右子节点
    private void spread(int node) {
        int todo = tree[node].todo;
        if (todo == 0) return;
        apply(node * 2, todo);       // 左儿子
        apply(node * 2 + 1, todo);   // 右儿子
        tree[node].todo = 0;         // 清空当前节点懒标记
    }

    // 合并左右子节点信息到当前节点
    private void maintain(int node) {
        tree[node].min = Math.min(tree[node * 2].min, tree[node * 2 + 1].min);
        tree[node].max = Math.max(tree[node * 2].max, tree[node * 2 + 1].max);
    }

    // 递归区间更新
    private void update(int node, int l, int r, int ql, int qr, int f) {
        if (ql <= l && r <= qr) { // 完全覆盖
            apply(node, f);
            return;
        }
        spread(node); // 下传懒标记
        int m = (l + r) / 2;
        if (ql <= m) {
            update(node * 2, l, m, ql, qr, f);
        }
        if (qr > m) {
            update(node * 2 + 1, m + 1, r, ql, qr, f);
        }
        maintain(node); // 更新当前节点信息
    }

    // 递归查找第一个等于 target 的位置
    private int findFirst(int node, int l, int r, int ql, int qr, int target) {
        // 剪枝：区间不交 或 target 不在 [min, max] 范围内
        if (l > qr || r < ql || target < tree[node].min || target > tree[node].max) {
            return -1;
        }
        if (l == r) { // 叶子节点，必然是答案
            return l;
        }
        spread(node);
        int m = (l + r) / 2;
        // 优先查左子树（保证“第一个”）
        int idx = findFirst(node * 2, l, m, ql, qr, target);
        if (idx < 0) {
            idx = findFirst(node * 2 + 1, m + 1, r, ql, qr, target);
        }
        return idx;
    }
}

class Solution {
    public int longestBalanced(int[] nums) {
        int n = nums.length;
        // 线段树维护前缀和数组 sum[0..n]，共 n+1 个元素
        LazySegmentTree t = new LazySegmentTree(n + 1);

        // last[x] = x 上一次出现的位置（1-indexed）
        Map<Integer, Integer> last = new HashMap<>();
        int ans = 0;
        int curSum = 0; // 当前前缀和（从 sum[0]=0 开始）

        // i 从 1 到 n，对应 nums[i-1]
        for (int i = 1; i <= n; i++) {
            int x = nums[i - 1];
            // 将奇数映射为 +1，偶数映射为 -1
            int v = (x % 2 == 1) ? 1 : -1;

            Integer j = last.get(x);
            if (j == null) {
                // 首次遇到 x：从位置 i 开始，所有后续前缀和都要 +v
                curSum += v;
                t.update(i, n, v); // sum[i..n] += v
            } else {
                // 再次遇到 x：撤销上一次对 [j, i-1] 的 +v（因为现在 x 出现两次，中间部分不能再贡献）
                t.update(j, i - 1, -v);
                // 注意：第三次及以后出现时，会继续撤销之前的贡献，确保只有最近两次有效
            }
            last.put(x, i); // 更新 x 的最新位置

            // 关键优化：只在 [0, i-1-ans] 范围内查找
            // 因为我们要找更长的子数组，所以 l 必须 ≤ i-1-ans，否则长度 ≤ ans
            int l = t.findFirst(0, i - 1 - ans, curSum);
            if (l >= 0) {
                ans = i - l; // 子数组 [l, i-1] 长度为 i - l
            }
        }
        return ans;
    }
}
```



## Python代码

```python
class LazySegmentTree:
    class Node:
        __slots__ = ('min_val', 'max_val', 'todo')
        def __init__(self):
            self.min_val = 0
            self.max_val = 0
            self.todo = 0

    def __init__(self, n: int):
        self.n = n
        # 计算树大小：2 << ceil(log2(n))
        if n <= 1:
            size = 2
        else:
            import sys
            bits = n - 1
            leading_zeros = (bits).bit_length()
            size = 2 << (32 - leading_zeros) if sys.maxsize > 2**32 else 2 << (64 - leading_zeros)
            # 更简单且安全的方式：直接用 4 * n
            size = 4 * n
        self.tree = [self.Node() for _ in range(size)]

    def _apply(self, node: int, todo: int):
        cur = self.tree[node]
        cur.min_val += todo
        cur.max_val += todo
        cur.todo += todo

    def _spread(self, node: int):
        todo = self.tree[node].todo
        if todo == 0:
            return
        self._apply(node * 2, todo)
        self._apply(node * 2 + 1, todo)
        self.tree[node].todo = 0

    def _maintain(self, node: int):
        left = self.tree[node * 2]
        right = self.tree[node * 2 + 1]
        self.tree[node].min_val = min(left.min_val, right.min_val)
        self.tree[node].max_val = max(left.max_val, right.max_val)

    def _update(self, node: int, l: int, r: int, ql: int, qr: int, f: int):
        if ql <= l and r <= qr:
            self._apply(node, f)
            return
        self._spread(node)
        m = (l + r) // 2
        if ql <= m:
            self._update(node * 2, l, m, ql, qr, f)
        if qr > m:
            self._update(node * 2 + 1, m + 1, r, ql, qr, f)
        self._maintain(node)

    def update(self, ql: int, qr: int, f: int):
        self._update(1, 0, self.n - 1, ql, qr, f)

    def _find_first(self, node: int, l: int, r: int, ql: int, qr: int, target: int) -> int:
        if l > qr or r < ql or target < self.tree[node].min_val or target > self.tree[node].max_val:
            return -1
        if l == r:
            return l
        self._spread(node)
        m = (l + r) // 2
        idx = self._find_first(node * 2, l, m, ql, qr, target)
        if idx < 0:
            idx = self._find_first(node * 2 + 1, m + 1, r, ql, qr, target)
        return idx

    def find_first(self, ql: int, qr: int, target: int) -> int:
        return self._find_first(1, 0, self.n - 1, ql, qr, target)


class Solution:
    def longestBalanced(self, nums: List[int]) -> int:
        n = len(nums)
        t = LazySegmentTree(n + 1)

        last: Dict[int, int] = {}
        ans = 0
        cur_sum = 0

        for i in range(1, n + 1):
            x = nums[i - 1]
            v = 1 if x % 2 == 1 else -1
            j = last.get(x)
            if j is None:
                cur_sum += v
                t.update(i, n, v)
            else:
                t.update(j, i - 1, -v)
            last[x] = i

            l = t.find_first(0, i - 1 - ans, cur_sum)
            if l >= 0:
                ans = i - l

        return ans   
```



## C++代码

```C++
class LazySegmentTree {
private:
    struct Node {
        int min_val = 0;
        int max_val = 0;
        int todo = 0;
    };

    void apply(int node, int todo) {
        tree[node].min_val += todo;
        tree[node].max_val += todo;
        tree[node].todo += todo;
    }

    int n;
    vector<Node> tree;

    void spread(int node) {
        int todo = tree[node].todo;
        if (todo == 0) return;
        apply(node * 2, todo);
        apply(node * 2 + 1, todo);
        tree[node].todo = 0;
    }

    void maintain(int node) {
        tree[node].min_val = min(tree[node * 2].min_val, tree[node * 2 + 1].min_val);
        tree[node].max_val = max(tree[node * 2].max_val, tree[node * 2 + 1].max_val);
    }

    void update(int node, int l, int r, int ql, int qr, int f) {
        if (ql <= l && r <= qr) {
            apply(node, f);
            return;
        }
        spread(node);
        int m = (l + r) / 2;
        if (ql <= m) {
            update(node * 2, l, m, ql, qr, f);
        }
        if (qr > m) {
            update(node * 2 + 1, m + 1, r, ql, qr, f);
        }
        maintain(node);
    }

    int findFirst(int node, int l, int r, int ql, int qr, int target) {
        if (l > qr || r < ql || target < tree[node].min_val || target > tree[node].max_val) {
            return -1;
        }
        if (l == r) {
            return l;
        }
        spread(node);
        int m = (l + r) / 2;
        int idx = findFirst(node * 2, l, m, ql, qr, target);
        if (idx < 0) {
            idx = findFirst(node * 2 + 1, m + 1, r, ql, qr, target);
        }
        return idx;
    }

public:
    LazySegmentTree(int n) : n(n) {
        int size = 2 << (32 - __builtin_clz(n - 1));
        tree.resize(size);
    }

    void update(int ql, int qr, int f) {
        update(1, 0, n - 1, ql, qr, f);
    }

    int findFirst(int ql, int qr, int target) {
        return findFirst(1, 0, n - 1, ql, qr, target);
    }
};

class Solution {
public:
    int longestBalanced(vector<int>& nums) {
        int n = nums.size();
        LazySegmentTree t(n + 1);

        unordered_map<int, int> last;
        int ans = 0;
        int curSum = 0;

        for (int i = 1; i <= n; i++) {
            int x = nums[i - 1];
            int v = (x % 2 == 1) ? 1 : -1;
            auto it = last.find(x);
            if (it == last.end()) {
                curSum += v;
                t.update(i, n, v);
            } else {
                t.update(it->second, i - 1, -v);
            }
            last[x] = i;

            int l = t.findFirst(0, i - 1 - ans, curSum);
            if (l >= 0) {
                ans = i - l;
            }
        }
        return ans;
    }
};
```

![3721_最长平衡子数组 II_C++](https://gitee.com/lei-weijun/CodingLearning/raw/main/DailyAlgorithm/LeetCode/8_%E5%B8%B8%E7%94%A8%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84/Images/3721_%E6%9C%80%E9%95%BF%E5%B9%B3%E8%A1%A1%E5%AD%90%E6%95%B0%E7%BB%84%20II_C++.png)
