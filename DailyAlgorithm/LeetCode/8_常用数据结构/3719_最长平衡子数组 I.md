本文题解基于以下原题整理，题意经本人重新表述，解题思路与代码均为原创或转载，仅用于个人学习与交流，完整题目请务必查阅官方原题链接：[3719. 最长平衡子数组 I - 力扣（LeetCode）](https://leetcode.cn/problems/longest-balanced-subarray-i/description/?envType=daily-question&envId=2026-02-10)
...
给定一个整数数组 `nums`，对于一个连续非空的子数组，如果其中包含的**互不相同的偶数**的个数等于**互不相同的奇数**的个数，则称该子数组是**平衡的**。请找出并返回数组中**最长平衡子数组**的长度。
...
class LazySegmentTree {
    ...
    public LazySegmentTree(int n) {
        ...
        // 例如 n=5 → n-1=4 (100) → leadingZeros=29 → 32-29=3 → 2<<3 = 16
...
> ⚠️ **重要提醒**：这段代码是为 **「最长平衡子数组 II」**（有数字频次限制）设计的。
> 而 **本题（I）没有频次限制**，所以这个解法**虽然能过，但严重过度设计**！
...
> 没有任何其他限制！同一个数字可以出现任意多次。

### 💡 核心思想：**前缀和 + 哈希表**

- 定义：`sum[i] = (奇数个数 - 偶数个数)` 从 `0` 到 `i-1`
- 映射：奇数 → `+1`，偶数 → `-1`
- 若 `sum[r+1] == sum[l]`，则子数组 `[l, r]` 是平衡的（和为 0）
- 用哈希表记录**每个前缀和第一次出现的位置**，以获得最长子数组
...
sum=0 出现在 -1, 1, 3 → 最长：3 - (-1) = 4 → [1,2,3,4]
```
...
1. 将奇数视为 `+1`，偶数视为 `-1`；
2. 计算前缀和 `sum`；
3. 若 `sum` 之前出现过，则 `[first[sum]+1, i]` 是平衡子数组；
4. 用哈希表记录每个 `sum` **第一次**出现的下标（以最大化长度）。
...
| -------------- | ------------------------- | ---------- |
| **I（本题）**  | 前缀和 + 哈希表           | O(n)       |
| **II（进阶）** | 懒标记线段树 + 动态前缀和 | O(n log n) |

> ✅ 对于 **本题（I）**，请使用 **简洁的前缀和解法**，不要用线段树！
...
![3719_最长平衡子数组 I_Java](https://gitee.com/lei-weijun/CodingLearning/raw/main/DailyAlgorithm/LeetCode/8_%E5%B8%B8%E7%94%A8%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84/Images/3719_%E6%9C%80%E9%95%BF%E5%B9%B3%E8%A1%A1%E5%AD%90%E6%95%B0%E7%BB%84%20I_Java.png)
