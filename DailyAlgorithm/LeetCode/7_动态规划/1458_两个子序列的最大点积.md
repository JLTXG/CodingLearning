本文题解基于以下原题整理，题意经本人重新表述，解题思路与代码均为原创或转载，仅用于个人学习与交流，完整题目请务必查阅官方原题链接：[1458. 两个子序列的最大点积 - 力扣（LeetCode）](https://leetcode.cn/problems/max-dot-product-of-two-subsequences/description/?envType=daily-question&envId=2026-01-08)
...
给定两个整数数组 `nums1` 和 `nums2`，你需要从每个数组中各选出一个**长度相同**的**非空子序列**，并计算这两个子序列的**点积**（即对应位置元素乘积之和）。目标是使这个点积值尽可能大，返回该最大点积。

**子序列**的定义：通过删除原数组中任意个元素（可以不删除）得到的序列，且剩余元素的相对顺序保持不变。

## **示例说明**

- **示例1**：`nums1 = [2,1,-2,5]`, `nums2 = [3,0,-6]`。选择 `nums1` 的子序列 `[2,-2]` 和 `nums2` 的子序列 `[3,-6]`，点积为 `2*3 + (-2)*(-6) = 6 + 12 = 18`。
- **示例2**：`nums1 = [3,-2]`, `nums2 = [2,-6,7]`。选择 `nums1` 的子序列 `[3]` 和 `nums2` 的子序列 `[7]`，点积为 `3*7 = 21`。
- **示例3**：`nums1 = [-1,-1]`, `nums2 = [1,1]`。选择 `nums1` 的子序列 `[-1]` 和 `nums2` 的子序列 `[1]`，点积为 `-1`。

## **数据范围**

- 数组长度满足 `1 <= nums1.length, nums2.length <= 500`。
- 数组中的每个元素满足 `-1000 <= nums1[i], nums2[i] <= 1000`。
...
![点积](https://gitee.com/lei-weijun/CodingLearning/raw/main/DailyAlgorithm/LeetCode/7_%E5%8A%A8%E6%80%81%E8%A7%84%E5%88%92/Images/1458_%E4%B8%A4%E4%B8%AA%E5%AD%90%E5%BA%8F%E5%88%97%E7%9A%84%E6%9C%80%E5%A4%A7%E7%82%B9%E7%A7%AF_%E7%82%B9%E7%A7%AF.png)
...
![1458_两个子序列的最大点积_Python](https://gitee.com/lei-weijun/CodingLearning/raw/main/DailyAlgorithm/LeetCode/7_%E5%8A%A8%E6%80%81%E8%A7%84%E5%88%92/Images/1458_%E4%B8%A4%E4%B8%AA%E5%AD%90%E5%BA%8F%E5%88%97%E7%9A%84%E6%9C%80%E5%A4%A7%E7%82%B9%E7%A7%AF_Python.png)
